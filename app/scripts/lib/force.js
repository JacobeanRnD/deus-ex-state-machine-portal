...
(function() {
  var CELL_MIN, CELL_PAD, CONTROL_RADIUS, DEBUG_FORCE_FACTOR, LABEL_SPACE, LINK_DISTANCE, LINK_STRENGTH, MARGIN, MAX_ZOOM, MIN_ZOOM, ROUND_CORNER, arrange, collide, def, exit, force, handleCollisions, move, nextId, parents, path, strip, treeFromXML, walk;
  ...
  force.drawTree = function(container, defs, tree, debug) {
    ...
    render = function() {
      ...
      container.selectAll('.cell').each(function(node) {
        return d3.select(this).select('rect').attr('x', -node.w / 2).attr('y', -node.h / 2).attr('width', node.w).attr('height', node.h);
      });
      container.selectAll('.cell').each(function(node) {
  ...
  collide = function(node, tick) {
    ...
    fn = function(quad, x1, y1, x2, y2) {
      var d, dx, dx1, dx2, dy, dy1, dy2, f, h, m, na, oa, other, s, s1, s2, w;
      other = quad.point;
      if (other && (other !== node)) {
        dx = node.x - other.x;
        dy = node.y - other.y;
  ...
  force.render = function(parent, doc, width, height) {
    var container, debug, defs, svg, tree, zoom, zoomNode, zoomRect;
    tree = treeFromXML(doc).sc;
    zoom = d3.behavior.zoom().scaleExtent([MIN_ZOOM, MAX_ZOOM]);
    svg = d3.select(parent).append('svg').classed('force-layout', true).classed('debug', debug);
